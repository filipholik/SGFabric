#!/usr/bin/python

from mininet.net import Mininet
from mininet.node import Controller, RemoteController, OVSController
from mininet.node import CPULimitedHost, Host, Node
from mininet.node import OVSKernelSwitch, UserSwitch
from mininet.node import IVSSwitch
from mininet.cli import CLI
from mininet.log import setLogLevel, info
from mininet.link import TCLink, Intf
from subprocess import call
import time 

from eBPFSwitch import eBPFSwitch, eBPFHost

def smartGridSimNetwork():

    net = Mininet( topo=None,
                   build=False,
                   ipBase='1.0.0.0/8',
                   host = eBPFHost, 
                   switch = eBPFSwitch)

    info( '\n*** ************************************* *** \n' )
    info( '*** Starting Three Tier Topology *** \n' )    
    info( '*** Version: 241203 \n' )
    info( '*** Author: filip.holik@glasgow.ac.uk  \n' )
    info( '*** ************************************* *** \n' )
    #info( '*** Adding controller\n' )
    #c0=net.addController(name='c0',
    #                  controller=OVSController,
    #                  protocol='tcp',
    #                  port=6633)

    switchPath = "../../BPFabric/softswitch/softswitch"; 

    switchType = OVSKernelSwitch; 

    info( '*** Starting networking devices\n')
    C1 = net.addSwitch('C1', switch_path=switchPath, dpid=1)    
    C2 = net.addSwitch('C2', switch_path=switchPath, dpid=2)    
    D1 = net.addSwitch('D1', switch_path=switchPath, dpid=3)
    D2 = net.addSwitch('D2', switch_path=switchPath, dpid=4) 
    D3 = net.addSwitch('D3', switch_path=switchPath, dpid=5) 
    D4 = net.addSwitch('D4', switch_path=switchPath, dpid=6) 
    A1 = net.addSwitch('A1', switch_path=switchPath, dpid=7) 
    A2 = net.addSwitch('A2', switch_path=switchPath, dpid=8) 
    A3 = net.addSwitch('A3', switch_path=switchPath, dpid=9) 
    A4 = net.addSwitch('A4', switch_path=switchPath, dpid=10) 

    info( '*** Starting hosts \n')
    H1 = net.addHost('DSS1RTU', cls=eBPFHost, ip='10.0.0.1', defaultRoute='10.10.10.10',mac='00:00:00:00:00:01')
    H2 = net.addHost('DSS2RTU', cls=eBPFHost, ip='10.0.0.2', defaultRoute='10.10.10.10',mac='00:00:00:00:00:02')
    H3 = net.addHost('CONTROL', cls=eBPFHost, ip='10.0.0.3', defaultRoute='10.10.10.10',mac='00:00:00:00:00:03')
    H4 = net.addHost('IED1', cls=eBPFHost, ip='10.0.0.4', defaultRoute='10.10.10.10',mac='00:00:00:00:00:04')
    H5 = net.addHost('IED2', cls=eBPFHost, ip='10.0.0.5', defaultRoute='10.10.10.10',mac='00:00:00:00:00:05')
    H6 = net.addHost('IED3', cls=eBPFHost, ip='10.0.0.6', defaultRoute='10.10.10.10',mac='00:00:00:00:00:06')
    H7 = net.addHost('IED4', cls=eBPFHost, ip='10.0.0.7', defaultRoute='10.10.10.10',mac='00:00:00:00:00:07')
    H8 = net.addHost('DPSHMI', cls=eBPFHost, ip='10.0.0.8', defaultRoute='10.10.10.10',mac='00:00:00:00:00:08')
    SERVER = net.addHost('SERVER', cls=eBPFHost, ip='10.10.10.10', defaultRoute='10.10.10.10',mac='00:00:00:00:00:10')

    info( '*** Setting link parameters\n')
    #WAN1 = {'bw':1000,'delay':'20ms','loss':1,'jitter':'10ms'} 
    #GBPS = {'delay':'18ms'} 
    MBPS = {'bw':1} 

    info( '*** Adding links\n')
    net.addLink(H1, A1, cls=TCLink , **MBPS)
    net.addLink(H2, A1, cls=TCLink , **MBPS)
    net.addLink(H3, A2, cls=TCLink , **MBPS)
    net.addLink(H4, A2, cls=TCLink , **MBPS)
    net.addLink(A1, D1, cls=TCLink , **MBPS)
    net.addLink(A2, D2, cls=TCLink , **MBPS)
    net.addLink(D1, C1, cls=TCLink , **MBPS)
    net.addLink(D2, C1, cls=TCLink , **MBPS)
    
    net.addLink(H5, A3, cls=TCLink , **MBPS)
    net.addLink(H6, A3, cls=TCLink , **MBPS)
    net.addLink(H7, A4, cls=TCLink , **MBPS)
    net.addLink(H8, A4, cls=TCLink , **MBPS)
    net.addLink(A3, D3, cls=TCLink , **MBPS)
    net.addLink(A4, D4, cls=TCLink , **MBPS)
    net.addLink(D3, C2, cls=TCLink , **MBPS)
    net.addLink(D4, C2, cls=TCLink , **MBPS)

    net.addLink(C1, C2, cls=TCLink , **MBPS)
    net.addLink(D4, SERVER, cls=TCLink , **MBPS)

    info( '*** Starting network\n')
    net.build()
    info( '*** Starting controllers\n')
    for controller in net.controllers:
        controller.start()

    info( '*** Starting networking devices\n')
    net.get('C1').start([])
    net.get('C2').start([])
    net.get('D1').start([])
    net.get('D2').start([])
    net.get('D3').start([])
    net.get('D4').start([])
    net.get('A1').start([])
    net.get('A2').start([])
    net.get('A3').start([])
    net.get('A4').start([])

    info( '*** Preparing custom scripts \n')
    CLI.do_attack_dos = attack_dos
    info( '*** Three Tier Topology Started *** \n' )
    CLI(net)

    net.stop()

def attack_dos(self, line):
    "Starts the DoS attack to the server." 
    net = self.mn  
    info('Starting DoS attack... \n')    
    net.get('H1').cmdPrint('xterm -geometry 90x30+10+10 -fa "Monospace" -fs 12 -T "ATTACKER-DOS" -e "sudo hping3 -S --flood 10.10.10.10;bash"&') 

if __name__ == '__main__':
    setLogLevel( 'info' )
    smartGridSimNetwork()





