...
class storage():
    ...
    cache = {}
    log = {str(datetime.datetime.now()): "WAN orchestrator started"}
...
class eBPFCLIApplication(eBPFCoreApplication):
    ...
    def load_balance(self):
        print("High traffic detected, enabling the backup path ") 
        with open('../functions/arp_block.o', 'rb') as f:
        ...
        with open('../functions/forwarding.o', 'rb') as f:
            ...
            storage.connections[14].send(FunctionAddRequest(name="learningswitch", index=2, elf=elf)) # W2
...
def threaded_mon_timer():
    while True:
        storage.connections[13].send(TableListRequest(index=1, table_name="monitor")) #D4 
...
def install(): 
    ...
    if(len(storage.connected_devices) == 14): 
        ...
        with open('../functions/forwarding.o', 'rb') as f:
            ...
            storage.connections[11].send(FunctionAddRequest(name="learningswitch", index=2, elf=elf)) # D5
            storage.connections[12].send(FunctionAddRequest(name="learningswitch", index=2, elf=elf)) # A5
            storage.connections[13].send(FunctionAddRequest(name="learningswitch", index=2, elf=elf)) # W1
            #storage.connections[14].send(FunctionAddRequest(name="learningswitch", index=2, elf=elf)) # W2
            time.sleep(1)    
        ...
        with open('../functions/monitoring.o', 'rb') as f:
            ...
            elf = f.read() 
            storage.connections[13].send(FunctionAddRequest(name="monitor", index=1, elf=elf)) #W1   
            #storage.connections[14].send(FunctionAddRequest(name="monitor", index=1, elf=elf)) #0   
            start_monitoring()
            time.sleep(1)
        ...
        print(f'Could not verify connected devices. Connected devices: ' + str(len(storage.connected_devices)))
        len(storage.connected_devices)
